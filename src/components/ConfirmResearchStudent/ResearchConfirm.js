...
class ConfirmStudent extends React.Component {
	...
	showDeleteConfirm(id, e) {
		...
		function deleteByConfirm(id) {
			fetch(config.api + '/Research/deleteByConfirm?id=' + id, {
				method: 'get',
				mode: 'cors',
				credentials: "include",
				headers: {
	...
	confirmToggle(id,e){
		e.preventDefault();
		fetch(config.api + '/Research/confirmStudent?id=' + id, {
			method: 'get',
			mode: 'cors',
			credentials: "include",
			headers: {
	...
	//获取该课题下 所有报名学生
	getStudent = (project_id) => {
		fetch(config.api + '/Research/getStudentByProject?project_id=' + project_id, {
			method: 'get',
			mode: 'cors',
			credentials: "include",
			headers: {
